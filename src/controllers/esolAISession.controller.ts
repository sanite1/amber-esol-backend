...
} from "../services/esolSessionToken.service";
...
export const generateSessionToken: ExpressFunction = async (req, res, next) => {
  try {
    const params = req.params as Record<string, string>;
    const data = await generateSessionTokenService({
      sessionId: params.sessionId,
      callerId: req.user!.id.toString(),
      callerRole: req.user!.role,
      callerOrgId: req.user!.orgId,
...
export const joinSession: ExpressFunction = async (req, res, next) => {
  try {
    const body = req.body as any;
    const data = await validateSessionTokenService({
      rawToken: body.token,
      learnerId: req.user!.id.toString(),
    });
    return res.status(200).json(data);
