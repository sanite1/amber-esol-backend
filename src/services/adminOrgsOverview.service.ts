...
export const getAdminOrgsOverview =
  async (): Promise<AdminOrgsOverviewResponse> => {
    const now = new Date();
    ...
    const rows: PipelineRow[] = await Organisation.aggregate([
      ...
      // ── Project to a flat row shape ───────────────────────────────
      {
        $project: {
          ...
          contractEnd: 1,
          learner_count: {
            $ifNull: [{ $arrayElemAt: ["$learners.total", 0] }, 0],
          },
    ...
    const orgs: AdminOrgsOverviewRow[] = rows.map((r) => {
      ...
      const sessionFeesRaw =
        r.is_demo === true ? 0 : (r.session_fees_this_month ?? 0);
